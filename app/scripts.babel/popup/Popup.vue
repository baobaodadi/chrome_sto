      ...
      <div class="input" v-if="setModeChecked">
        <el-form :inline="true" ref="formInline" :model="formInline" class="demo-form-inline" size="mini" :rules="rules">
          <el-form-item label="股票代码" prop="code">
            <el-autocomplete v-model="formInline.code" :fetch-suggestions="querySearch" :trigger-on-focus="false" @select="handleSelect" placeholder="请输入股票代码或股票名" clearable>
            </el-autocomplete>
          </el-form-item>
          <el-form-item label="成本价">
            <el-input v-model="formInline.cost" placeholder="请输入持仓成本" clearable>
            </el-input>
          </el-form-item>
          <br>
          <el-form-item label="持股量">
            <el-input v-model="formInline.count" placeholder="请输入持股量" clearable>
            </el-input>
          </el-form-item>
          <el-form-item>
...
export default {
	...
	methods: {
		...
		formatter(row, column) {
			return row && row.range ? row.range.toFixed(2) + '%' : '0'
		},
		...
		edit(index) {
			// 定义两个变量 减少引用次数
			let { cost, count, upLimit, downLimit } = this.localStock[index]
			// 点击完成
			if (this.stocks[index].edit) {
				this.$set(this.stocks[index], 'edit', false)
				this.$set(
					this.localStock[index],
					'notifiedTime',
					new Date().getTime() - 13 * 60 * 60 * 1000
				)
				let temProfit =
					this.stocks[index].curPrice === 0 || cost === 0
						? 0
						: getFixedNum((this.stocks[index].curPrice - cost) * count, 3)
				this.stocks[index] = Object(this.stocks[index], {
					cost: cost || 0,
					count: count || 0,
					upLimit: upLimit || 0,
      ...
      count = count ? count : 0
		...
		_getStockByCode(code, cost = 0, count = 0, upLimit = 0, downLimit = 0) {
			...
			getStockByCode(code).then(res => {
				...
				if (stockObj) {
					let idxOfStocks = this.stocks.indexOfAtt(code, 'code')
					let idxOfLocalStock = this.localStock.indexOfAtt(code, 'code')
            ...
            console.log({ code, cost, count, upLimit, downLimit })
